...
import { getBidSectionByTenProjList } from '@/api/supervise'
export default {
    components: { search },
    mixins: [searchQuery],
    data() {
        return {
            ...
            titles: [
                ...
                { name: '所属招标项目', data: 'TenderProjectName' },
    ...
    created() {
        this.searchData.bidSectionCode = this.$route.query.bidSectionCodes
        this.init()
    },
    methods: {
        init() {
            ...
            getBidSectionByTenProjList(data).then((res) => {
        ...
        getVal(val) {
            if (val.type === '详情') {
                ...
                this.$router.push({
                    path: '/supervise/bidinfo',
                    query: { SId: val.data.SId, bidSectionCodes: val.data.BidSectionCode, pageType }
                })
            } else return
        }
    }
