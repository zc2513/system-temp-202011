...
<template>
  <div>
    <z-header title="应届生汇报" class="mb15">
      <SelcetStudent :show-button="true" :user-id="userInfo.id" :selected="false" @changeUser="changeUser" />
    </z-header>
    <el-row :gutter="6">
      <el-col :span="12">
        <tabel-header img-name="person" :title="orgName" :num="userCount" />
      </el-col>
      <el-col :span="12">
        <tabel-header type="1" />
      </el-col>
    ...
    >
      <template v-slot:tsdateCell="{ data }">
        ...
        <div class="bottom">
          ...
          <z-circle size="20" :color=" countSave(data.data.day)>=userCount?'#D697FF':countSave(data.data.day)>0?'#A2F07B':'#FF785F' ">
            {{ countSave(data.data.day)>= userCount ?'all': countSave(data.data.day) }}
          </z-circle>
        </div>
      </template>
      <template v-slot:weekContent="{ week }">
        ...
        <div class="bottom">
          ...
          <z-circle size="20" :color=" countWeekSave(week.item.week)>=userCount?'#D697FF':countWeekSave(week.item.week)>0?'#A2F07B':'#FF785F' ">
            {{ countWeekSave(week.item.week)>= userCount ?'all': countWeekSave(week.item.week) }}
          </z-circle>
        </div>
      </template>
      <template v-slot:monthContent="{ month }">
        ...
        <!-- <div class="bottom"></div> -->
        <div class="bottom">
          <z-circle size="20" :color=" countMonthSave(month.item.month)>=userCount?'#D697FF':countMonthSave(month.item.month)>0?'#A2F07B':'#FF785F' ">
            {{ countMonthSave(month.item.month)>= userCount ?'all': countMonthSave(month.item.month) }}
          </z-circle>
...
import SelcetStudent from '@/components/tsforce/SelectStudent.vue'
export default {
    name: 'Myplan',
    components: {
        lock,
    ...
    methods: {
        ...
        changeUser(selectorg, selectUser, selectOrgName, selectNode, userCount) {
            console.log('选中的用户组织架构与用户', selectorg, selectUser, selectOrgName, selectNode, userCount)
            ...
            console.log('选中的用户组织架构与用户----------', this.userCount, this.groupId)
        }
    }
}
</script>
