<template>
  <div>
    ...
    <el-card>
      <el-row>
        ...
        <el-col :span="6">
          ...
          <span style="width: 80px" v-if="tabPosition === 'self'">{{ currentMonth.monthString }}</span>
          <span style="width: 80px" v-if="tabPosition === 'week'">{{ currentSeason.seasonString }}</span>
          <span style="width: 80px" v-if="tabPosition === 'month'">{{ currentYear.desc }}</span>
    ...
    <show-week-plan-modal ref="showWeekPlan"></show-week-plan-modal>
    <add-plan-modal ref="addDayPlan"></add-plan-modal>
  </div>
</template>
          ...
          this.weeks[this.currentSeason.seasonNo - 1].weekPlan.forEach(element => {
            if (weekPlan.length > 0) {
              for (var i = 0; i < weekPlan.length; i++) {
                let weekint = parseInt(weekPlan[i].week)
